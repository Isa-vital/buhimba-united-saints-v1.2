...
<section class="results-hero position-relative text-white py-5">
    ...
    <div class="container position-relative">
        <div class="row align-items-center">
            ...
            <div class="col-lg-4 text-lg-end">
                <div class="bg-white bg-opacity-20 backdrop-blur rounded p-3">
                    <h5 class="mb-1">Last Match</h5>
                    @if($lastResult)
                    <div class="fw-bold">
                        @if($lastResult->is_home)
                        Buhimba United Saints {{ $lastResult->home_score }}-{{ $lastResult->away_score }} {{ $lastResult->opponent }}
                        @else
                        {{ $lastResult->opponent }} {{ $lastResult->home_score }}-{{ $lastResult->away_score }} Buhimba United Saints
                        @endif
                    </div>
    ...
    <div class="container">
        ...
        <div class="mb-5">
            ...
            <div class="row">
                ...
                @php
                // Determine if we won, drew, or lost based on the actual database structure
                if ($result->is_home) {
                $isWin = $result->home_score > $result->away_score;
                $isDraw = $result->home_score == $result->away_score;
                $homeTeam = 'Buhimba United Saints FC';
                $awayTeam = $result->opponent;
                } else {
                $isWin = $result->away_score > $result->home_score;
                $isDraw = $result->home_score == $result->away_score;
                $homeTeam = $result->opponent;
                $awayTeam = 'Buhimba United Saints FC';
                }
                $isLoss = !$isWin && !$isDraw;
                @endphp
                            ...
                            <div class="row align-items-center text-center mb-3">
                                <div class="col-4">
                                    <div class="team-info">
                                        ...
                                        @endif
                                        <h6 class="fw-bold mb-0 {{ $result->is_home ? 'text-club-green' : '' }}">
                                            {{ $homeTeam }}
                                        </h6>
                                    </div>
                                </div>
                                    ...
                                    <div class="score-section">
                                        <div class="score-display bg-light rounded p-3">
                                            <h2 class="fw-bold mb-0">
                                                <span class="{{ $result->is_home && $result->home_score > $result->away_score ? 'text-success' : '' }}">
                                                    {{ $result->home_score }}
                                                </span>
                                                <span class="text-muted mx-2">-</span>
                                                <span class="{{ !$result->is_home && $result->away_score > $result->home_score ? 'text-success' : '' }}">
                                                    {{ $result->away_score }}
                                                </span>
                                            </h2>
...
@push('styles')
<style>
    /* Results Hero Section with Background Image */
    .results-hero {
        min-height: 60vh;
        position: relative;
        overflow: hidden;
